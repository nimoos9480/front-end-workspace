...
- 사용자가 사용하는 브라우저는 정해져 있지 않아서 브라우저마다 화면이 다르게 출력이 될 수 있으니 브라우저마다 테스트를 하면서 수정해야 함
    => 크로스 브라우징
...
width="가로 길이(px/%)" height="세로길이(px/%)">
```

## img 태그 속성의 alt

- 사진의 경로가 잘못되거나 이미지를 제대로 표현할 수 없는 경우 대체 텍스트의 용도
- 웹 접근성에 의해 정보를 청각, 촉각으로 얻는 사람들을 위해 img 속서의 alt로 정보를 전달한다.
...
```html
<video src="비디오 경로" controls autoplay loop 
width="" height="" poster="썸네일 이미지 경로">
```
...
2. caption : 테이블의 제목을 뜻함
3. thead(제목) / tfoot(결과) / tbody(내용) : 각 셀들의 역할별 구분이 가능
